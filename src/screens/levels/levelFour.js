...
import {chapterNumber, levelNumber} from '../../utils/constants';
const dWidth = Dimensions.get('screen').width;
const dHeight = Dimensions.get('screen').height;
export default levelFour = ({navigation, route}) => {
  ...
  const onSubmit = async () => {
    ...
    await AsyncStorage.setItem(
      chapterNumber,
      (parseInt(hCheapterNumber) + 1).toString(),
    );
    navigation.goBack();
  };
    ...
    <View style={{flex: 1, backgroundColor: colors.creamBase1}}>
      ...
      <ScrollView style={{paddingHorizontal: 20}}>
        ...
        <View style={styles.titleContainer}>
          <Text style={styles.titletext}>Left hand - Level 2</Text>
          <Text style={styles.subtitletext}>
            Use the previous to find the bassline and notes by ear, and play
            them on your piano.
          </Text>
        </View>
        <View style={styles.playContainer}>
          ...
          {hideNodes ? (
            ...
            <View style={{height: 50, marginVertical: 25}} />
          )}
          <Pressable onPress={() => setHideNodes(!hideNodes)}>
